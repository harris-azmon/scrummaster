...
description: Scaffolds the project and sets up the Scrummaster environment. Use this whenever a project needs to be initialized or if the Scrummaster configuration is missing.
metadata:
  version: "1.1"
---
...
-   **Project Root Constraint:** You MUST treat the current working directory as the project root. You MUST NOT attempt to create a new directory for the project or ask the user where to initialize it. All Scrummaster artifacts must be stored within a `scrummaster/` directory in the current project root. If you detect that the current directory is not suitable (e.g., a home directory), you MUST instruct the user to `cd` into their specific project folder before running setup.
...
1.  **Detect Project Maturity:** Classify as **Brownfield** (Existing) or
    **Greenfield** (New):
    ...
    -   **Brownfield Indicators:**
        ...
        -   **Fossil Hygiene:** If an open Fossil checkout exists (`fossil info`
            succeeds), execute `fossil changes`. Ignore changes within
            `scrummaster/`. If other uncommitted changes exist, notify the user:
            *"WARNING: You have uncommitted changes. Please commit or stash them
            before proceeding."* and classify as Brownfield.
    -   **Greenfield Condition:** Classify as Greenfield ONLY if:
        ...
        -   The directory contains no application source code or dependency
            manifests (ignoring `scrummaster/`, a clean/newly initialized Fossil
            checkout, and a `README.md`).
...
- **Efficient Scan:** Upon permission, analyze the project while minimizing token usage:
    - Use `fossil ls` to identify relevant files (or `fossil extras` for untracked ones worth noting).
    - Respect `.fossil-settings/ignore-glob` (fossil's ignore-pattern file) and `.geminiignore` patterns.
...
- **Initialize Fossil:** If no open checkout exists (`fossil info` fails), run
  `fossil init <name>.fossil && fossil open <name>.fossil`, then apply the
  Scrummaster ticket schema: `fossil sql < templates/fossil/ticket_schema.sql`.
...
3. Once approved, create the `scrummaster/` directory (if missing) and write the final content to `scrummaster/product.md`.
...
3. **Action:** Once approved, write the final content to `scrummaster/product-guidelines.md`.
...
3.  **Action:** Once approved, write the final content to `scrummaster/tech-stack.md`.

### 2.4 Code Style Guides

Select and copy appropriate style guides from `assets/code_styleguides/` to the project root at `scrummaster/code_styleguides/`.
...
4. **Write Action:** Copy `assets/workflow.md` to `scrummaster/workflow.md` and apply user choices if customized.
...
1. **Analyze Needs & Trust Model:**
    ...
    - **Trust Disclosure:** For each recommendation, disclose the `Party` status:
        - **1p (Official):** Present as a verified, official Scrummaster skill.
...
Create `scrummaster/index.md` and `scrummaster/epics.md`. `index.md` is the
**Single Source of Truth** for all tools; `epics.md` is the (initially empty)
epic → story index, synced from fossil ticket state on every future setup/status/
implement run.
...
4.  **Publish to Fossil Wiki:** Fossil's wiki is the source of truth for these
    documents; the files on disk are a synced mirror. Create the wiki pages:
    ```bash
...
5.  **Commit Stage:** Stage the entire `scrummaster/` directory. Create a commit
    with the message: `scrummaster(setup): Initialize project context and
    standards` (`fossil add scrummaster && fossil commit -m "..."`, direct to
    trunk per the Cathedral-style default).

## 4. Completion

Once the `scrummaster/` directory is created and the index is generated, announce that setup is complete.
...
2.  **Proactive Suggestion:** Ask the user if they would like to start defining
    ...
    -   **Greenfield (New Project):** Ask if they want to start planning the
        initial epic and its first story (MVP) right now.
    -   **Brownfield (Existing Project):** Ask if they want to start defining
        their first actionable epic and story (feature, bug fix, or chore) right now.
3. **Internal Handoff:** If the user agrees, you MUST use the `scrummaster-new-epic`
   skill first (a story always belongs to an epic), then hand off to
   `scrummaster-new-story` to begin planning within it.
