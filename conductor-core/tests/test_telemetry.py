...
from conductor_core.telemetry import TelemetryLogger
...
def test_log_implementation_attempt_writes_entry(tmp_path):
    ...
    log_file = logger.log_implementation_attempt("track-1", data)

    assert log_file.exists()
    assert log_file.parent == tmp_path
    assert log_file.name.startswith("implement_track-1_")
