import pytest
from conductor_core.config import ConductorConfig, ConfigManager
...
def test_load_config_creates_default_when_missing(tmp_path):
    ...
    assert (tmp_path / "conductor" / "config.json").exists()


def test_load_config_reads_existing_file(tmp_path):
    conductor_dir = tmp_path / "conductor"
    conductor_dir.mkdir()
    (conductor_dir / "config.json").write_text('{"project_name": "my-project"}', encoding="utf-8")
...
def test_load_config_invalid_json_raises(tmp_path):
    conductor_dir = tmp_path / "conductor"
    conductor_dir.mkdir()
    (conductor_dir / "config.json").write_text("not json", encoding="utf-8")
...
def test_load_config_validation_error_raises(tmp_path):
    conductor_dir = tmp_path / "conductor"
    conductor_dir.mkdir()
    (conductor_dir / "config.json").write_text('{"enable_locking": "not-a-bool"}', encoding="utf-8")
...
def test_save_config_with_explicit_config(tmp_path):
    manager = ConfigManager(tmp_path)
    config = ConductorConfig(project_name="explicit")
