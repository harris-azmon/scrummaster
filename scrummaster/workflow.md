...
9. **Attach Task Summary with a Fossil Technote:**

    - **Step 9.1: Get Commit Hash:** Obtain the hash of the *just-completed
        commit* (`fossil info | awk '/^checkout:/ {print $2}'`).
...
10. **Get and Record Task Commit Hash:**
    ...
    - **Step 10.1: Update Plan:** Read `plan.md`, find the line for the
        ...
        first 10 characters of the *just-completed commit's* hash.
...
3. **Logical State Reversions (`scrummaster-revert`):** If a task implementation
    ...
    manually in compatible clients). Since Scrummaster commits directly to trunk
    by default, this generates a new forward commit from an inverse patch (fossil
    has no `git revert` equivalent — see `templates/vcs_workflows/fossil.md` →
    `revert_commit`), reopens the task's ACID ticket(s), and resets the task
    state in `plan.md` back to pending `[ ]` to allow a clean restart.
...
2. **Ensure Test Coverage for Phase Changes:**
    ...
    - **Step 2.1: Determine Phase Scope:** To identify the files changed in
        ...
        commit.
    - **Step 2.2: List Changed Files:** Execute `fossil diff --from
        <previous_checkpoint_hash> --to current --brief` to get a precise list
        of all files modified during this phase.
...
7. **Attach Auditable Verification Report using a Fossil Technote:**
...
8. **Get and Record Phase Checkpoint Hash:**
    ...
    - **Step 8.1: Get Commit Hash:** Obtain the hash of the *just-created
        checkpoint commit* (`fossil info | awk '/^checkout:/ {print $2}'`).
    - **Step 8.2: Update Plan:** Read `plan.md`, find the heading for the
        completed phase, and append the first 10 characters of the commit hash in
...
9. Fossil technote with task summary attached to the commit

## Emergency Procedures

### Critical Bug in Production

Cathedral-style trunk development has no feature branches by default, so hotfixes
commit directly to trunk like any other task:

1. Commit a failing test for the bug directly to trunk
2. Implement the minimal fix
3. Test thoroughly including mobile
4. Tag the fix commit for traceability: `fossil tag add hotfix-<date> trunk`
...
Trunk-oriented by default — releases are cut from trunk directly, not merged in
from a branch:

1. Tag the release check-in: `fossil tag add release-<version> trunk`
2. Push to the deployment service
