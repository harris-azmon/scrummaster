#!/usr/bin/env python3
"""Scrummaster Update Checker and Updater

Checks for updates and applies them to scrummaster components.

Usage:
    python scripts/scrummaster_update.py [--check-only] [--all] [--component COMPONENT]
"""
...
class UpdateChecker:
    """Checks and applies updates to scrummaster components"""
    ...
    def get_current_version(self, component: str) -> str | None:
        ...
        if component == "core":
            ...
            code, stdout, _stderr = self.run_command(
                [sys.executable, "-c", "import scrummaster_gemini; print(scrummaster_gemini.__version__)"]
            )
            if code == 0:
                return stdout.strip()
    ...
    def get_latest_github_release(self, repo: str) -> str | None:
        ...
        try:
            ...
            req.add_header("User-Agent", "scrummaster-updater")
    ...
    def get_latest_git_tag(self, component: str) -> str | None:
        ...
        component_dir = self.base_path / f"scrummaster-{component}"
    ...
    def _update_core(self) -> bool:
        """Update scrummaster-core"""
        core_path = self.base_path / "scrummaster-core"
        if not core_path.exists():
            self.log("[WARN]  scrummaster-core not found", Colors.YELLOW)
            return True
    ...
    def _update_gemini(self) -> bool:
        """Update scrummaster-gemini"""
        gemini_path = self.base_path / "scrummaster-gemini"
        if not gemini_path.exists():
            self.log("[WARN]  scrummaster-gemini not found", Colors.YELLOW)
            return True
    ...
    def _update_vscode(self) -> bool:
        ...
        if not vscode_path.exists():
            self.log("[WARN]  scrummaster-vscode not found", Colors.YELLOW)
            return True
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check for and apply scrummaster updates",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python scripts/scrummaster_update.py --check-only    # Check only, don't update
  python scripts/scrummaster_update.py --all           # Update everything
  python scripts/scrummaster_update.py --core          # Update core only
        """,
    )
    ...
    parser.add_argument(
        "--core",
        action="store_true",
        help="Update scrummaster-core only",
    )
    parser.add_argument(
        "--gemini",
        action="store_true",
        help="Update scrummaster-gemini only",
    )
    parser.add_argument(
        "--vscode",
        action="store_true",
